...
def read_matrix(input_file):
    ...
    if not path.exists():
        raise FileNotFoundError(f"Epigenome matrix not found: {path}")
...
def clip_beta_values(df, sample_id_column):
    out = df.copy()
...
def median_impute(df, sample_id_column):
    out = df.copy()
...
def resolve_output_paths(manifest):
    ...
    feature_store_dir = Path(
        expected_outputs.get(
            "feature_store_dir",
            f"outputs/features/epigenome/{atlas_name}",
        )
    )
...
def process_epigenome_manifest(manifest_path):
    manifest_path = Path(manifest_path)
    manifest = validate_manifest_file(manifest_path)

    if manifest["modality"] != "epigenome":
        raise ValueError(f"Manifest modality must be epigenome, got: {manifest['modality']}")
    ...
    imputed_df = median_impute(filtered_df, sample_id_column)
    clipped_df = clip_beta_values(imputed_df, sample_id_column)
    ...
    feature_store_manifest = write_feature_store_manifest(
        ...
        feature_store_id=f"{manifest['atlas_name']}_epigenome_feature_store",
        atlas_name=manifest["atlas_name"],
        modality="epigenome",
        ...
        agent_purpose="provide epigenomic features for downstream AI multi-omics analysis",
    )
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Process an epigenome matrix from a validated data manifest into a feature store."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Epigenome data manifest YAML.",
    )
...
def main(argv=None):
    ...
    try:
        normalized_df, _, _, paths = process_epigenome_manifest(args.manifest)
    except Exception as exc:
        print(f"ERROR: Epigenome processing failed: {exc}", file=sys.stderr)
        return 1

    print("Epigenome feature-store processing complete.")
