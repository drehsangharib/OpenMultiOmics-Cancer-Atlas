...
def read_matrix(input_file):
    ...
    if not path.exists():
        raise FileNotFoundError(f"Metabolomics matrix not found: {path}")
...
def log_transform_then_scale(df, sample_id_column):
    out = df.copy()
    ...
    for column in out.columns:
        ...
        values = out[column].clip(lower=0)
        ...
        out[column] = (values - mean_value) / std_value
...
def apply_normalization(df, sample_id_column, normalization):
    if normalization in {"log_transform_then_scale", "log2_then_zscore", "log_scale"}:
        return log_transform_then_scale(df, sample_id_column)

    if normalization in {"median_centering", "median_center"}:
        return median_center(df, sample_id_column)

    if normalization in {"none", "", None}:
        return df.copy()

    raise ValueError(f"Unsupported metabolomics normalization: {normalization}")
...
def resolve_output_paths(manifest):
    ...
    feature_store_dir = Path(
        expected_outputs.get(
            "feature_store_dir",
            f"outputs/features/metabolomics/{atlas_name}",
        )
    )
...
def process_metabolomics_manifest(manifest_path):
    manifest_path = Path(manifest_path)
    manifest = validate_manifest_file(manifest_path)

    if manifest["modality"] != "metabolomics":
        raise ValueError(f"Manifest modality must be metabolomics, got: {manifest['modality']}")
    ...
    filtered_df, high_missingness_removed = remove_high_missingness_features(
        numeric_df,
        sample_id_column,
    ...
    normalized_df = apply_normalization(
        imputed_df,
        sample_id_column,
        processing_plan.get("normalization", "log_transform_then_scale"),
    )

    normalized_df, zero_variance_removed = remove_zero_variance_features(
        normalized_df,
        sample_id_column,
    )
    ...
    qc_summary_df = build_qc_summary(
        raw_df,
        numeric_df,
        filtered_df,
        normalized_df,
    ...
    feature_store_manifest = write_feature_store_manifest(
        ...
        feature_store_id=f"{manifest['atlas_name']}_metabolomics_feature_store",
        atlas_name=manifest["atlas_name"],
        ...
        agent_purpose="provide metabolomic features for downstream AI multi-omics analysis",
    )
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Process a metabolomics matrix from a validated data manifest into a feature store."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Metabolomics data manifest YAML.",
    )
...
def main(argv=None):
    ...
    try:
        normalized_df, _, _, paths = process_metabolomics_manifest(args.manifest)
    except Exception as exc:
        print(f"ERROR: Metabolomics processing failed: {exc}", file=sys.stderr)
        return 1

    print("Metabolomics feature-store processing complete.")
