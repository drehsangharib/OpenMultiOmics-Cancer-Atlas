...
Purpose:
    ...
    availability, and project priority rankings.
...
def summarize_programs(project_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if project_df.empty:
        return pd.DataFrame(columns=["program_name", "project_count", "case_count", "file_count"])
...
def dataframe_to_html_table(
    ...
    if columns is not None:
        display_df = df.loc[:, [col for col in columns if col in df.columns]].copy()
    else:
        display_df = df.copy()
...
def build_report_html(
    ...
    excellent_count = int(
        (ranking_df["priority_label"] == "excellent").sum()
...
<div class="note">
    ...
    and project priority scores for cancer multi-omics atlas construction.
    Generated outputs are local analysis artifacts and are not committed by default.
</div>
...
<ul>
    <li>Priority scores reflect metadata availability, not biological quality or suitability for a specific hypothesis.</li>
    ...
    <li>Future versions may include visual modality heatmaps, interactive dashboards, and cancer-specific atlas reports.</li>
</ul>
</div>
