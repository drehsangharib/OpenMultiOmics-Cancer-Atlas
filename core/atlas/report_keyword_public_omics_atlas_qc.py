...
def default_input_path(atlas_name):
    return DEFAULT_ATLAS_ROOT / atlas_name / f"{atlas_name}_public_omics_atlas_inventory.tsv"


def default_output_path(atlas_name):
    return DEFAULT_REPORT_ROOT / f"{atlas_name}_public_omics_atlas_qc_report.html"


def read_keyword_atlas_inventory(input_path):
    if not input_path.exists():
        raise FileNotFoundError(
            f"Keyword atlas inventory not found: {input_path}. "
            "Run python -m core.atlas.build_keyword_public_omics_atlas first."
        )
...
def build_qc_report_html(df, atlas_name, title=None):
    if title is None:
        title = f"{atlas_name.upper()} Public Omics Atlas QC Report"
    ...
    match_counts = value_counts_table(df, "atlas_match_terms", max_rows=30)
    ...
    html_parts = [
        ...
        f"<p>This report summarizes quality-control checks for the <strong>{escape_html(atlas_name)}</strong> public omics atlas inventory.</p>",
...
def generate_keyword_public_omics_atlas_qc_report(
    atlas_name,
    input_path=None,
    output_path=None,
    title=None,
    ...
    if input_path is None:
        input_path = default_input_path(atlas_name)

    if output_path is None:
        output_path = default_output_path(atlas_name)

    df = read_keyword_atlas_inventory(input_path)
    report_html = build_qc_report_html(df, atlas_name=atlas_name, title=title)
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Generate QC report for a keyword-driven public omics atlas inventory."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional atlas inventory TSV input path.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional QC HTML output path.",
    )
...
def main(argv=None):
    ...
    try:
        ...
        print(f"ERROR: Failed to generate keyword atlas QC report: {exc}", file=sys.stderr)
        return 1

    output_path = args.output if args.output is not None else default_output_path(args.atlas_name)

    print(f"{args.atlas_name.upper()} public omics atlas QC report complete.")
