...
Purpose:
    ...
    ranking and project modality matrix outputs.
...
Outputs:
    ...
    outputs/figures/gdc_gdc_pipeline_schematic.png
...
def build_top_project_heatmap_matrix(
    ...
    """
    Build a project × modality matrix for the top-ranked projects.
    """
...
def plot_project_modality_heatmap(
    ...
    """
    Plot a project × modality heatmap for top-ranked projects.
    """
    ...
    fig_height = max(6, len(matrix.index) * 0.28)

    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    image = ax.imshow(matrix.values, aspect="auto", interpolation="nearest")

    ax.set_title(f"Top {len(matrix)} GDC Projects × Modality Availability")
    ...
    fig.colorbar(image, ax=ax, fraction=0.025, pad=0.02, label="Available")
    save_figure(fig, output_path)
    return output_path
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of top projects to include in modality heatmap.",
    )
