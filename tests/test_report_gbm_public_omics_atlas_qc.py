...
import atlases.gbm.report_gbm_public_omics_atlas_qc as qc
...
def test_qc_metrics():
    metrics = qc.build_qc_metrics(make_gbm_df())
...
def test_missingness_table():
    table = qc.build_missingness_table(make_gbm_df())
...
def test_value_counts_table():
    table = qc.value_counts_table(make_gbm_df(), "source_id")
...
def test_unknown_modality_table():
    table = qc.build_unknown_modality_table(make_gbm_df())
...
def test_missing_source_url_table():
    table = qc.build_missing_source_url_table(make_gbm_df())
...
def test_source_modality_coverage_table():
    table = qc.build_source_modality_coverage_table(make_gbm_df())
...
def test_build_qc_report_html():
    report_html = qc.build_qc_report_html(make_gbm_df())
...
def test_generate_gbm_public_omics_atlas_qc_report(tmp_path: Path):
    input_path = tmp_path / "gbm.tsv"
    output_path = tmp_path / "gbm_qc.html"

    make_gbm_df().to_csv(input_path, sep="\t", index=False)

    report_html = qc.generate_gbm_public_omics_atlas_qc_report(
