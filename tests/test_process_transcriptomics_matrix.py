...
from core.modalities.process_transcriptomics_matrix import (
    build_arg_parser,
    process_transcriptomics_manifest,
)


def write_manifest(path: Path, matrix_path: Path, output_dir: Path):
    manifest = {
        "manifest_id": "test_transcriptomics_manifest",
        "atlas_name": "brca",
        ...
        "agent_role": {
            ...
            "purpose": "prepare transcriptomic features for downstream AI multi-omics analysis",
        },
    }
...
def test_process_transcriptomics_manifest(tmp_path: Path):
    ...
    matrix = pd.DataFrame(
        {
            "sample_id": ["S1", "S2", "S3"],
            "GENE1": [1.0, 2.0, 3.0],
            "GENE2": [10.0, None, 30.0],
            "ZERO_VAR": [5.0, 5.0, 5.0],
            "NON_NUMERIC": ["a", "b", "c"],
        }
    )
    ...
    normalized_df, qc_summary_df, feature_store_manifest, paths = process_transcriptomics_manifest(
        manifest_path
    )
    ...
    assert "GENE1" in normalized_df.columns
    assert "GENE2" in normalized_df.columns
    ...
    assert feature_store_manifest["modality"] == "transcriptomics"
